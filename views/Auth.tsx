...
/** Gridline-based layout — all divisions end at same top/bottom/center */
    ...
    <div className={`view-transition flex-1 min-h-0 flex items-center justify-center p-4 lg:p-6 overflow-auto ${isDark ? 'bg-black' : 'bg-[#F0F0F0]'}`}>
      <div className={`relative w-full max-w-[900px] overflow-hidden px-4 py-6 sm:px-8 sm:py-8 md:px-10 md:py-10 ${isDark ? 'bg-[#0a0a0a]' : 'bg-transparent'}`}>
        ...
        <div className="flex flex-col lg:flex-row min-h-0">
          {/* LEFT: Brand — back lives in the panel so it aligns with the card chrome */}
            ...
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-3">
                <PageBackButton
                  ...
                  fallbackTo="/"
                  label="Back"
                  className={`bb-auth-back ${
                    isDark
                      ? 'bg-white/5 border-white/10 text-white hover:bg-white/10 hover:border-[#CDA032]/40'
                      : 'bg-black/[0.04] border-black/10 text-black hover:bg-black/[0.07]'
                  }`}
                />
                <div className={`h-5 w-px flex-shrink-0 ${isDark ? 'bg-white/15' : 'bg-black/15'}`} aria-hidden />
                <div className="flex min-w-0 items-center gap-2.5">
                  ...
                  <span className={`text-sm font-black tracking-widest uppercase italic truncate ${leftText}`}>BLACK BOX.</span>
                </div>
              </div>
