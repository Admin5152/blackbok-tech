...
Manual verification for the client-side **10-minute visible** inactivity logout (`SessionTimeoutProvider` / `useIdleLogout`).

Minimizing or hiding the tab **pauses** the idle clock — time in the background does not count. Logout only after ~10 minutes of no input while the tab is visible.
...
- [ ] Sign in and stay idle (no mouse/keyboard/scroll) for 10 minutes with the tab visible → signed out and redirected to `/#/auth?reason=session_expired`.
...
- [ ] Moving the mouse or pressing a key before 10 minutes resets the timer (no logout).
- [ ] Minimize / switch away for >10 minutes, then return → still signed in (idle clock was paused).
- [ ] Navigating between protected routes (e.g. `/profile` → `/history`) does **not** reset or leak the timer; idle still fires at 10m from last real activity while visible.
...
- [ ] Sign in, leave tab **visible** and idle, put machine to sleep for >10 minutes, wake → logout occurs shortly after wake if the tab is still considered visible idle.
