-- =====================================================================
-- Persist cart variant_id onto order_items + place_order INSERT
-- Migration: 20260813001000_order_items_variant_id_place_order.sql
--
-- Body matches 2026_05_place_order_rpc_customer_fields.sql; only change
-- is writing variant_id from cart JSON so 00900 decrement can hit SKUs.
-- Idempotent.
-- =====================================================================
...
COMMENT ON COLUMN public.order_items.variant_id IS
  'Selected product_variants.id when the shopper checked out a SKU config.';
...
BEGIN
  ...
  FOR v_item IN SELECT jsonb_array_elements(p_cart_items) LOOP
    ...
    BEGIN
      v_vid_uuid := NULLIF(BTRIM(COALESCE(v_vid_text, '')), '')::UUID;
    EXCEPTION WHEN invalid_text_representation THEN
      v_vid_uuid := NULL;
    END;
